...
### Fixed — batch mode leaked and muted every IDA dialog

After prolonged MCP use, IDA would stop showing **any** modal dialog — the `g`
jump-to-address box, the script chooser, even the save-on-close prompt — logging an
error to the Output window instead. Only a restart cleared it.

Batch mode (`idc.batch(1)`) suppresses dialogs so a tool call can never block the UI
waiting on user input, but it is a **global** flag and was leaking two ways:

1. Save/restore ran per-call from the **RPC worker thread**, outside `execute_sync`.
   Two overlapping requests interleave as
   `A:batch(1)→0 │ B:batch(1)→1 │ A:restore 0 │ B:restore 1` — stuck ON. (It was also
   an IDA API call off the main thread, which IDA's threading model forbids.)
2. `call_stack.get()` blocked on an empty queue when a reentrant `@idasync` had already
   popped the entry, parking the main thread and stranding the restore
   ([ida-pro-mcp#406](https://github.com/mrexodia/ida-pro-mcp/issues/406)).

The toggle now happens inside `execute_sync` (main thread, serialized) and is
reference-counted against one saved outer value; both `call_stack` pops are
non-blocking. A **`ui_unstick`** tool is included as a recovery hatch — it should never
be needed, but if dialogs ever die again it restores them without losing the database.

Everything above was verified against a live IDA 9.4 instance (search, all four
signature formats with round-trip uniqueness, and concurrent-request stress); the
router / registry / instance-routing / idalib layer is untouched.
...
pipx install git+https://github.com/bromoket/ida-multi-mcp.git

# 2. Install package for IDA's Python (replace 3.11 with your IDA's version)
python3.11 -m pip install --user git+https://github.com/bromoket/ida-multi-mcp.git
...
python -m pip install git+https://github.com/bromoket/ida-multi-mcp.git
...
```bash
# 1. Install ida-multi-mcp
pip install --user git+https://github.com/bromoket/ida-multi-mcp.git
...
2. Install the package for that specific Python version:
   ...
   **Windows:**
   ...
   py -3.12 -m pip install git+https://github.com/bromoket/ida-multi-mcp.git
   ```
